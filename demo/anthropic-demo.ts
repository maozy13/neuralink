...
async function main(): Promise<void> {
  ...
  const firstResponse = await consume(connector.call(model, prompt, { tools }));
  ...
  console.log("\nAnthropic 最终 Response:");
  console.log(JSON.stringify(finalResponse, null, 2));
}

/**
 * Consumes and prints an Anthropic response stream.
 * @param iterator Response stream to consume.
 * @returns The accumulated normalized response.
 */
async function consume(iterator: ReturnType<Connector<unknown, unknown>["call"]>): Promise<Response> {
  let eventCount = 0;
  while (true) {
...
/**
 * Provides deterministic weather data for a city-specific weather tool.
 * @param name Name of the selected weather tool.
...
/**
 * Creates the weather tools used by the parallel-call demo.
